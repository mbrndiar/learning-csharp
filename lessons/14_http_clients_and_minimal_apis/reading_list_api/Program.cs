...
static async Task RunSmokeAsync(WebApplication app)
{
    ...
    try
    {
        ...
        // Contrast: `client` above is a raw HttpClient this method created, used, and
        // will dispose itself. The typed client below is resolved through
        // IHttpClientFactory instead, so a container - not this method - owns the
        // underlying handler, its DNS-aware lifetime, and its reuse across calls.
        int typedClientBookCount = await GetBookCountThroughTypedClientAsync(baseAddress, CancellationToken.None);

        if (books.Books.Count != 2
            || missingBook is not null
            || invalidPost.StatusCode != HttpStatusCode.BadRequest
            || typedClientBookCount != 2)
        {
            throw new InvalidDataException("Smoke mode observed unexpected HTTP behavior.");
        }

        Console.WriteLine(
            $"SMOKE TEST PASSED books={books.Books.Count} seeded={seededBook.Title} missing=404 invalidPost={(int)invalidPost.StatusCode} typedClientBooks={typedClientBookCount}");
    }
    finally
    {
        await app.StopAsync();
...
static async Task<int> GetBookCountThroughTypedClientAsync(string baseAddress, CancellationToken cancellationToken)
{
    ...
    // This is a small, separate DI container built only to demonstrate typed-client
    // resolution; it is not the sample app's own request-processing container, so
    // the usual "don't build your own provider inside the app" analyzer warning
    // does not apply here.
#pragma warning disable ASP0000
    await using ServiceProvider provider = services.BuildServiceProvider();
#pragma warning restore ASP0000
    SampleTypedBookClient typedClient = provider.GetRequiredService<SampleTypedBookClient>();
    SampleBookListResponse books = await typedClient.GetBooksAsync(cancellationToken);
...
/// <summary>
/// A typed client: DI supplies the <see cref="HttpClient"/> instance through
/// <see cref="IHttpClientFactory"/>, so this class never constructs or disposes
/// one itself. The factory owns handler reuse, DNS-refresh behavior, and the
/// finite timeout configured where this type is registered.
/// </summary>
internal sealed class SampleTypedBookClient(HttpClient httpClient)
{
    public async Task<SampleBookListResponse> GetBooksAsync(CancellationToken cancellationToken)
    {
        using HttpResponseMessage response = await httpClient.GetAsync("/books", cancellationToken);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<SampleBookListResponse>(SampleJsonOptions.Value, cancellationToken)
            ?? throw new InvalidDataException("The sample API returned an empty book list.");
    }
}
