...
{
    public static CreateBookRequest ToCommand(this CreateBookRequestDto request) =>
        new(request.Title, request.Author, request.PublicationYear, request.Isbn);

    public static CreateReadingEntryRequest ToCommand(this CreateReadingEntryRequestDto request) =>
        new(request.BookId, request.StartedOn, request.FinishedOn, request.PagesRead, request.Rating, request.Notes);
