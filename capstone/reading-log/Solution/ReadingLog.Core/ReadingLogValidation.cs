...
public static class ReadingLogValidation
{
    public static CreateBookRequest ValidateCreateBookRequest(CreateBookRequest request)
    {
        ...
        if (request.PublicationYear is int publicationYear && (publicationYear < 1450 || publicationYear > maxYear))
        {
            AddError(errors, "publicationYear", $"Publication year must be between 1450 and {maxYear}.");
        }
    ...
    private static void ValidateBook(Book book, int index, Dictionary<string, List<string>> errors)
    {
        ...
        var maxYear = DateTime.UtcNow.Year + 1;
        if (book.PublicationYear is int publicationYear && (publicationYear < 1450 || publicationYear > maxYear))
        {
            AddError(errors, $"books[{index}].publicationYear", $"Publication year must be between 1450 and {maxYear}.");
        }
    }
